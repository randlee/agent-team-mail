...
| G.7 | TUI streaming normalization + daemon pubsub/UDP fanout architecture | G.4, G.6 | 🔄 IN PROGRESS |
...
**Execution model**: G.1–G.6 complete (PRs #168–#173). G.5 and G.6 ran in parallel after G.4. G.7 (TUI streaming/fanout) is in progress. G.2 (CLI-JSON verification) is deferred until after G.7 so streaming infrastructure is shared with app-server rather than validated in isolation. G.8-G.9 are reliability and release readiness.
