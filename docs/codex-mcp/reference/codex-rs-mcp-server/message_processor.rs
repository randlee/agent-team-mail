/*
REFERENCE COPY (from codex-rs/mcp-server) for atm-agent-mcp design review.

Use/adapt:
- structural patterns (dispatch, outgoing envelopes, schema generation, test harness)

Do not use as-is:
- codex-specific runtime/state internals
- external API naming as external contract (prefer agent_id in atm-agent-mcp)
- transport assumptions without validating atm-agent-mcp protocol requirements
*/
