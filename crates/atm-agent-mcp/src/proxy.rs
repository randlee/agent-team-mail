        ...
        loop {
            tokio::select! {
                ...
                result = reader.next_message() => {
                    ...
                    // Use a scoped block so the span guard is dropped before any
                    // .await point — EnteredSpan is !Send and must not be held
                    // across await points in a tokio::spawn context.
                    {
                        let _req_span =
                            tracing::debug_span!("mcp_request", request_id = %req_id).entered();
                        drop(_req_span);
                    }
