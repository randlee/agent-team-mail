#!/usr/bin/env python3
"""Notification(idle_prompt) hook relay for ATM daemon state tracking.

When Claude has been idle for a period, emit an idle lifecycle heartbeat.
...
def main() -> int:
    ...
    try:
        send_hook_event(
            {
                "event": "notification_idle_prompt",
